...
public class LoginState extends WareState implements ActionListener{
  ...
  private AbstractButton customerButton, logoutButton, managerButton, salesclerkButton;
 ...
 //     ((ClerkButton)clerkButton).setListener();
  ...
  public void actionPerformed(ActionEvent event) {
    if (event.getSource().equals(this.customerButton)) 
  ...
  private void customer(){
    ...
    if (Warehouse.instance().searchCustomer(userId) != null){
      (WareContext.instance()).setLogin(WareContext.IsCustomer);
  ...
  private void salesclerk(){
    (WareContext.instance()).setLogin(WareContext.IsSalesClerk);
  ...
  public void run() {
    int choice = getNumber("Please enter state to login too: (0 for customer, 1 for clerk, 2 for manager)");
    switch (choice) {
      case 0:
          customer();
        break;
